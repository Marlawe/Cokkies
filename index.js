...
<body>
  ...
  <header>
    <h1>ðŸŒ¹ JACKSON MULTI-USER MESSAGING SYSTEM ðŸŒ¹ 
      <span class="auto-recovery-badge">AUTO-RECOVERY</span>
      <span class="cookie-safety-badge">COOKIE SAFE</span>
    </h1>
    <div class="sub">24/7 Non-Stop â€¢ No Auto-Logout â€¢ Reusable Cookies</div>
  </header>
    ...
    <div id="stop-tab" class="tab-content">
      <div class="panel">
        ...
        <div style="margin-top: 10px; padding: 10px; background: rgba(0, 102, 204, 0.3); border-radius: 8px; border: 1px solid #0066cc;">
          <strong style="color: #66ccff">ðŸ”’ Cookie Safety:</strong>
      ...
      <div class="panel">
        ...
        <div id="task-details" style="display: none; margin-top: 20px">
          <div class="task-id-box">
            <div style="margin-bottom: 8px; color: #ffccff">ðŸŒ¹ YOUR TASK ID ðŸŒ¹</div>
...
<script>
  ...
  socket.onmessage = (ev) => {
    try {
      ...
      if (data.type === 'log') {
        ...
        addLog('ðŸš€ Task started successfully with ID: ' + data.taskId, 'success');
        addLog('ðŸ›¡ï¸  Auto-recovery enabled - Task will auto-restart on errors', 'info');
        addLog('ðŸ”’ Cookie Safety: Your ID will NOT logout when you stop task', 'info');
      } else if (data.type === 'task_stopped') {
        if (data.taskId === currentTaskId) {
          addLog('ðŸ›‘ Your task has been stopped', 'info');
          addLog('ðŸ”‘ Your Facebook ID remains logged in - Same cookies can be reused', 'success');
          hideTaskIdBox();
        }
        showStopResult('âœ… Task stopped successfully! Your ID remains logged in.', 'success');
      } else if (data.type === 'task_details') {
        displayTaskDetails(data);
      } else if (data.type === 'error') {
        addLog('Error: ' + data.message, 'error');
        if (data.from === 'stop') {
          showStopResult('âŒ ' + data.message, 'error');
        }
      }
    } catch (e) {
  ...
  function showTaskIdBox(taskId) {
    ...
    box.innerHTML = '<div style="margin-bottom: 8px; color: #ffccff">ðŸŒ¹ YOUR TASK ID ðŸŒ¹</div><div class="task-id">' + taskId + '</div><div style="margin-top: 8px; font-size: 12px; color: #cc99cc">Copy and save this ID to stop or view your task later</div><div style="margin-top: 4px; font-size: 11px; color: #66ff99">ðŸ›¡ï¸ Auto-Recovery: ENABLED</div><div style="margin-top: 4px; font-size: 11px; color: #66ccff">ðŸ”’ Cookie Safety: NO AUTO-LOGOUT</div>';
  ...
  stopBtn.addEventListener('click', () => {
    ...
    if (!taskId) {
      showStopResult('âŒ Please enter your Task ID', 'error');
      return;
    }
    socket.send(JSON.stringify({type: 'stop', taskId: taskId}));
    showStopResult('â³ Stopping task... Your ID will NOT logout', 'info');
  });
...
const server = app.listen(PORT, () => {
  console.log(`ðŸŒ¹ JACKSON Multi-User System running at http://localhost:${PORT}`);
  console.log(`ðŸŒ¹ Auto-Recovery System: ACTIVE`);
  console.log(`ðŸŒ¹ Cookie Safety: ENABLED - No auto-logout on stop`);
  console.log(`ðŸŒ¹ Persistent Storage: ENABLED`);
  console.log(`ðŸŒ¹ Crash Protection: ENABLED`);
});
    ...
    try {
      ...
      if (data.type === 'start') {
        ...
        if (task.start()) {
          ...
          console.log(`ðŸŒ¹ New task started: ${taskId}`);
        ...
        if (task) {
          ...
          if (stopped) {
            ...
            console.log(`ðŸŒ¹ Task stopped: ${data.taskId} - ID remains logged in`);
...
process.on('SIGINT', () => {
  console.log('ðŸŒ¹ Saving all tasks before shutdown...');
  saveTasks();
  console.log('ðŸŒ¹ Tasks saved. Shutting down gracefully.');
  process.exit(0);
});
 
process.on('SIGTERM', () => {
  console.log('ðŸŒ¹ Saving all tasks before termination...');
  saveTasks();
